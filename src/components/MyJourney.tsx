...
const MyJourney = ({ onBack, onNavigate }: MyJourneyProps) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Calculate streak from memorization data
    const practicedays = new Set<string>();
    memData.forEach((m: any) => {
      if (m.last_practiced) {
        practicedays.add(new Date(m.last_practiced).toISOString().split("T")[0]);
      }
    });
    readData.forEach((r: any) => {
      if (r.updated_at) {
        practicedays.add(new Date(r.updated_at).toISOString().split("T")[0]);
      }
    });
    ...
    if (readData.length > 0) {
      setLastPosition(readData[0]);
    }

    // Load last position from localStorage too
  ...
  // Heatmap rendering (last 12 weeks)
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center space-y-4 animate-fade-in">
            ...
            <p className={`text-muted-foreground ${isAr ? "font-arabic" : ""}`}>
              {isAr ? "سجّل الدخول لتتبع رحلتك" : "Sign in to track your journey"}
            </p>
          ...
          <>
            ...
            {lastPosition && (
              <button
                ...
                className="w-full bg-card border border-border rounded-xl p-4 hover:border-primary/40 transition-all text-left animate-slide-up"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div
                  key={i}
                  className="bg-card border border-border rounded-xl p-4 text-center animate-slide-up"
                ...
                {heatmapWeeks.map((week, wi) => (
                  <div key={wi} className="flex flex-col gap-1">
                    {week.map((day) => (
                      <div
                        key={day}
                        className={`w-3 h-3 rounded-[3px] transition-colors ${getHeatColor(heatmapData[day] || 0)}`}
                        title={`${day}: ${heatmapData[day] || 0} activities`}
                      />
                    ))}
                  </div>
                ))}
            ...
            {totalPracticed > 0 && (
              <div className="bg-accent/5 border border-accent/20 rounded-xl p-4 space-y-2 animate-slide-up" style={{ animationDelay: "400ms", animationFillMode: "both" }}>
                ...
                {(() => {
                  ...
                  return (
                    <p className={`text-xs text-muted-foreground ${isAr ? "font-arabic" : ""}`}>
                      {isAr ? "أنت تبلي بلاءً حسناً! استمر في الحفظ" : "You're doing great! Keep memorizing new ayahs"}
                    </p>
                  );
                })()}
              ...
              <div className="space-y-2">
                ...
                {memProgress.slice(0, 10).map((item, i) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="bg-card border border-border rounded-xl p-3 flex items-center gap-3 animate-slide-up"
