...
| **Saved library** | Polymorphic favourites for ayahs, recitations and AI answers; every saved item is re-openable — ayahs deep-link back into the Mushaf at the exact verse, recitations reopen in the player, and saved chats rehydrate inside the assistant so the conversation can continue | `lib/bookmarks.ts`, `components/FavoritesHub.tsx`, `components/ChatInterface.tsx` |
...
| **Navigation shell** | Frosted, safe-area-aware mobile tab bar with a raised Family action that fires a particle celebration on tap; grouped desktop/drawer navigation (Main · Family · Worship · Learn · You) generated from the shipped view registry so no entry can drift from a real screen | `components/BottomNav.tsx`, `components/Navbar.tsx`, `components/CelebrationBurst.tsx` |
...
| **Handoffs, not dead ends** | Reopening a saved item never shows a read-only copy: a one-shot `localStorage` handoff seeds the destination screen (e.g. a saved answer is replayed into the chat transcript, already marked as saved), so the user resumes rather than restarts. |
